...
const OUTPUT_PATH := "res://docs/art/previews/characters-combat/player-b-action-slice-combat-runtime-v1.png"
const VIEWPORT_SIZE := Vector2i(1280, 720)

func _initialize() -> void:
	print("RENDER START: Player B action slice combat preview")
	...
	scene.player.action_slice_preview_enabled = true
	scene.player.action_slice_current_action = "fire"
	scene.player.action_slice_elapsed = 0.18
	scene.player.action_slice_fire_timer = 0.5
	...
	if capture == null or capture.is_empty():
		push_error("Player B action slice combat capture is empty")
		quit(1)
		return
	var absolute_path := ProjectSettings.globalize_path(OUTPUT_PATH)
	DirAccess.make_dir_recursive_absolute(absolute_path.get_base_dir())
	if capture.save_png(absolute_path) != OK:
		push_error("Could not save player B action slice combat capture: " + OUTPUT_PATH)
		quit(1)
		return
	print("RENDER PASS: Player B action slice combat preview %dx%d -> %s" % [capture.get_width(), capture.get_height(), OUTPUT_PATH])
	quit(0)
