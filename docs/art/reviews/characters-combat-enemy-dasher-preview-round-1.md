...
- 当前门：用户已于 2026-08-09 选择同时保留 A 和 B；C 不进入正式制作。
- 资源状态：A、B 已进入 `style-approved`；本页中的三张图仍是黑蓝背景 RGB 选型预览。
...
- 保留 A：作为人形战术疾行兵变体。
- 保留 B：作为低重心废铁疾行机甲变体。
- 淘汰 C：保留预览和 manifest 作为决策记录，不进入正式资源目录。
- A、B 共用现有 Dasher 玩法数值；本轮只制作视觉资源，不修改碰撞、速度、生命、伤害或 AI。
