...
美术资源的预览、选择、生成、导入与验收以 `docs/art/2026-08-03-art-production-plan.md` 为准，并作为本玩法计划的表现层依赖。

- 每次决定角色、特效、环境、UI 或未来玩法主题的风格前，必须先提供 `A/B/C` 三个预览方向。
- 用户未明确选择前，不锁定风格、不扩展正式批次。
- 批次顺序固定为角色与战斗、环境、UI；上一批未获得用户批准和游戏内验收时，不进入下一批。
- 未来传送门、超载、终极进化和首领资源在对应玩法合同稳定后另开预览门。
- 美术替换不得改变碰撞、伤害半径、移动、阶段或 UI 行为。
